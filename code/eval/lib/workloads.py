...
class BfsExe(Workload):
    ...
    def get_cmdline(self, procs_path, pinned_cpus):
        ...
        shell_cmd = '/usr/bin/time -v' + ' ' + constants.WORK_DIR + '/GridGraph/bin/bfs /home/yanghanzhang/cfm/GridGraph/datasets/lj_4 0 100'
        pinned_cpus_string = ','.join(map(str, pinned_cpus))
        set_cpu = 'taskset -c {}'.format(pinned_cpus_string)
        full_command = ' '.join((prefix, 'exec', set_cpu, shell_cmd))
        return full_command
...
class BfsPre(Workload):
    ...
    def get_cmdline(self, procs_path, pinned_cpus):
        prefix = "echo $$ > {} &&".format(procs_path)
        shell_cmd = '/usr/bin/time -v' + ' ' + constants.WORK_DIR + '/GridGraph/bin/preprocess -i /home/yanghanzhang/cfm/GridGraph/datasets/lj_binary  -o /home/yanghanzhang/cfm/GridGraph/datasets/lj_4 -v 4847571 -p 4 -t 0'
        pinned_cpus_string = ','.join(map(str, pinned_cpus))
        set_cpu = 'taskset -c {}'.format(pinned_cpus_string)
        full_command = ' '.join((prefix, 'exec', set_cpu, shell_cmd))
        return full_command
